...
# 让用户输入存储路径（可选）
read -p "请输入节点存储数据的宿主机路径（直接回车将使用默认路径 /home/titan_storage_$i,依次数字顺延）: " custom_storage_path
...
docker pull registry.cn-hangzhou.aliyuncs.com/lvjinqiao/titan-edge
...
do
    ...
    # 运行容器，并设置重启策略为always
    container_id=$(docker run -d --restart always -v "$storage_path:/root/.titanedge/storage" --name "titan$i" registry.cn-hangzhou.aliyuncs.com/lvjinqiao/titan-edge)
